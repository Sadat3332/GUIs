...
class Pokedex:
    ...
    def create_pokemon_list(self):
        ...
        self.search_frame = Frame(self.pokemon_list_frame, bg='white', bd=5,
                                  highlightcolor="black", highlightbackground='black', highlightthickness=1)
        self.search_frame.pack(side='top')

        self.pokemon_search_entry = Entry(
            self.search_frame, bg='white', relief='groove', bd=3)
        self.pokemon_search_entry.pack(side='right', padx=10, pady=10)
        self.pokemon_search_entry.bind(
            '<KeyRelease>', self.filter_pokemon_list)

        search_icon = PhotoImage(
            file='images/pokedex-icon.png').subsample(7, 7)
        self.search_button = Button(
            self.search_frame, image=search_icon, bg='white', bd=0)
        self.search_button.image = search_icon
        self.search_button.pack(side='left', padx=(0, 10))

        self.scrollbar = ttk.Scrollbar(
            self.pokemon_list_frame, orient='vertical')
    ...
    def create_pokemon_image(self):
        ...
        self.pokelabel = Label(self.pokemon_display_frame,
                               image=self.pokeimage, bg='#f5edef')
        self.pokelabel.pack()
        self.pokelabel.configure(image=self.pokeimage, bg='#f5edef')

        self.text = Text(self.pokemon_display_frame,
                         height=10, width=45, bg='#e6e1e2')
    ...
    def create_attributes_bar(self):
        self.attribute_frame = Frame(
            self.info_tab, borderwidth=1, relief="groove", bg='white')
        self.attribute_frame.grid(
            row=0, column=1, padx=10, pady=10, sticky='nw')

        Label(self.attribute_frame, text="Base Stats",
              font=('Helvetica', 12, 'bold'), bg='white', pady=8, padx=5).grid(row=0, column=0, sticky='w')
        attributes = ['HP', 'Attack', 'Defense', 'Sp_Attack',
                      'Sp_Defense', 'Speed', 'Capture_Rate']
        colors = ['#08d141', '#c7042b', '#f76a05',
                  '#f705b3', '#f7f705', '#058ef7', '#51ff00']

        self.progress_bars = []
        for i, (attr, color) in enumerate(zip(attributes, colors)):
            val = int(
                round(self.poke_df.loc[self.curr_index, attr.lower()], 0))
            Label(self.attribute_frame, text=attr, padx=10, pady=10, font=(
                'Helvetica', 10, 'bold'), bg='white').grid(row=i+1, column=0, sticky='w')
            s = ttk.Style()
            s.theme_use('default')
        ...
        self.weight_label = Label(self.attribute_frame, text='Weight: ' + str(
            self.poke_df.loc[self.curr_index, 'weight_kg']) + ' kg', padx=10, pady=10, font=('Helvetica', 10, 'bold'), bg='white')
    ...
    def create_Info_tab(self):
        ...
        # Adjust subsample ratio as needed
        pokedexi = PhotoImage(file='images/pokemon_logo.png').subsample(10, 10)
        search_button = Button(self.info_frame_image,
                               image=pokedexi, bg='white', bd=0)
        search_button.image = pokedexi
        search_button.grid(row=0, column=0)

        self.pokemon_info_image_label = Label(
            self.info_frame_image, bg='white')
        self.pokemon_info_image_label.config(image=self.pokeimage)
        self.pokemon_info_image_label.grid(
        ...
        self.pokemon_classification = self.poke_df.loc[self.curr_index,
                                                       'classfication']
    ...
    def poke_selected(self):
        ...
        self.curr_name = self.pokemon_listbox.get(
            self.pokemon_listbox.curselection()[0])

        self.curr_index = int(self.curr_name[:3])-1
        self.pokemon_name_label.config(
            text=str(self.poke_df.loc[self.curr_index, 'name'][4:]))
        self.pokemon_name_label_info.config(
            text=str(self.poke_df.loc[self.curr_index, 'name'][4:]))
        self.pokemon_classification = self.poke_df.loc[self.curr_index,
                                                       'classfication']
        ...
        self.height_label.config(
            text='Height: ' + str(self.poke_df.loc[self.curr_index, 'height_m']) + ' m')
        self.weight_label.config(
            text='Weight: ' + str(self.poke_df.loc[self.curr_index, 'weight_kg']) + ' kg')

        self.pokemon_classification_label.config(
            text=self.pokemon_classification)
